...
import { ShoppingCart, Tractor, ChevronDown, ImagePlus, LoaderCircle } from 'lucide-react';

const DISTRICTS = [
  "Select your district",
  "Dhaka", "Bogura", "Rangpur", "Khulna", "Sylhet",
];
...
export default function RegisterPage() {
  ...
  // Improved base styles for mobile thumb-tapping
  ...
  return (
    <main className="min-h-screen bg-zinc-50 py-8 md:py-16 px-4 md:px-12">
      <div className="max-w-2xl mx-auto flex flex-col items-center">
        ...
        {/* Form Card - w-full is critical here */}
        <form onSubmit={handleSubmit} className="w-full p-5 md:p-10 bg-white rounded-[2rem] shadow-xl shadow-green-950/5 border border-zinc-100">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {/* Role Card Component (Abstracted logic for cleaner JSX) */}
              {[
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1 relative">
              <label className="text-xs font-semibold text-zinc-600 ml-1">District</label>
              <div className="relative">
                <select defaultValue={DISTRICTS[0]} className={`${inputBase} appearance-none pr-10 cursor-pointer`}>
                  {DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
                <ChevronDown className="absolute top-1/2 -translate-y-1/2 right-4 text-zinc-400 w-4 h-4 pointer-events-none" />
              </div>
            </div>

            {/* Profile Image Dropzone - Responsive sizing */}
            <div className="space-y-1">
          ...
          <div className="mt-8 space-y-4">
            ...
            >
              {loading ? <LoaderCircle className="w-5 h-5 animate-spin" /> : "Create Account"}
            </button>
